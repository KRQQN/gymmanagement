        ...
        .then(async (gymsData) => {
          const gymsWithClasses = await Promise.all(
            gymsData.map(async (gym: any) => {
              try {
                const classes = await ClassService.getAllClasses(gym.id);
                return {
                  ...gym,
                  classes: classes.map((cls: any) => ({
                    ...cls,
                    schedule: Array.isArray(cls.schedule) ? cls.schedule : [cls.schedule],
                  })),
                };
              } catch (error) {
                        ...
                        <div className="flex-1 space-y-4">
                          ...
                          <div>
                            ...
                            <ul className="space-y-1">
                              {Array.isArray(cls.schedule) && cls.schedule.map((slot: ScheduleSlot, index: number) => (
